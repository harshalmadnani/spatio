...
function ChatInterface() {
  ...
  const [isWalletPortfolioLoading, setIsWalletPortfolioLoading] =
    useState(true);
  ...
  const priceHistory = Object.entries(priceHistoryData).map(
    ([coinName, data]) => ({
      coinName,
      data: data.map((item) => ({
  ...
  const fetchPriceHistory = async (coinname, from = null, to = null) => {
    ...
    try {
      ...
      console.log(
        `Fetching price history for ${coinname} from ${new Date(
          from
        )} to ${new Date(to)}...`
      );
  ...
  const fetchCryptoPanicData = async (coinname) => {
    try {
      if (!coinname) {
        console.error(
          "Attempted to fetch CryptoPanic data with undefined coinname"
        );
        setErrorSnackbar({
          open: true,
      ...
      const coin = coins.find(
        (c) => c.name.toLowerCase() === coinname.toLowerCase()
      );
      if (!coin) {
      ...
      console.log(
        `Fetching CryptoPanic data for ${coinname} (${coin.symbol})...`
      );
      const response = await axios.get(
  ...
  const fetchHistoricPortfolioData = async (
    ...
    try {
      ...
      console.log(
        `Fetching historic portfolio data from: ${url}?${params.toString()}`
      );
  ...
  const fetchWalletPortfolio = async (addresses = walletAddresses) => {
    ...
    try {
      ...
      console.log(
        `Fetching wallet portfolio from: ${url}?${params.toString()}`
      );
...
6. When fetching news, use the renderCryptoPanicNews function and return its result directly.`,
        ...
        if (codeMatch && codeMatch[1]) {
          ...
          const finalResponse = await openai.chat.completions.create({
            ...
            messages: [
              {
                role: "user",
                content: `As spatio AI, provide an answer for the following query: "${userInput}". The data from the execution is: ${result}`,
              },
            ],
            temperature: 0.7,
            max_tokens: 3000,
          ...
          console.log(
            "Final response from GPT-4o-mini:",
  ...
  const executeCode = async (code) => {
    try {
      ...
      if (result === undefined) {
        throw new Error(
          "Execution result is undefined. Make sure the code returns a value."
        );
      }
  ...
  const renderMessage = (message, index) => {
    ...
    const executionResultMatch = content.match(
      /\*\*Execution Result:\*\*\n```\n([\s\S]*?)\n```/
    );
    const executionResult = executionResultMatch
      ? executionResultMatch[1]
      : null;
    ...
    if (newWalletAddress) {
      ...
      if (!isEthereumAddress && !isSolanaAddress) {
        setErrorSnackbar({
          open: true,
          message:
            "Invalid wallet address. Please enter a valid Ethereum or Solana address.",
        });
        return;
      }
  ...
  const updateWalletAddresses = async (updatedAddresses) => {
    ...
    try {
      ...
      if (!historicData || !walletData) {
        throw new Error(
          "Failed to load wallet data: One or both data fetches returned null or undefined"
        );
      }
      ...
      setErrorSnackbar({
        open: true,
        message: `Failed to update wallet data: ${error.message}`,
      });
  ...
  const volume = async (token) => {
    ...
    if (!metadata) return "please resend the prompt";
    return metadata.volume !== undefined
  ...
  const marketCapDiluted = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.market_cap_diluted !== undefined
  ...
  const liquidity = async (token) => {
    ...
    if (!metadata) return "please resend the prompt";
    return metadata.liquidity !== undefined
      ? `$${metadata.liquidity.toFixed(2)}`
      : "N/A";
  };
  const liquidityChange24h = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.liquidity_change_24h !== undefined
  ...
  const offChainVolume = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.off_chain_volume !== undefined
  ...
  const volume7d = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.volume_7d !== undefined
      ? `$${data.volume_7d.toFixed(2)}`
      : "N/A";
  };
  const volumeChange24h = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.volume_change_24h !== undefined
  ...
  const isListed = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.is_listed !== undefined
      ? data.is_listed
        ? "Yes"
        : "No"
      : "N/A";
  ...
  const priceChange24h = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.price_change_24h !== undefined
  ...
  const priceChange1h = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.price_change_1h !== undefined
  ...
  const priceChange7d = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.price_change_7d !== undefined
  ...
  const priceChange1m = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.price_change_1m !== undefined
  ...
  const priceChange1y = async (token) => {
    ...
    if (!data) return "please resend the prompt";
    return data.price_change_1y !== undefined
  ...
  const totalSupply = async (token) => {
    ...
    if (!metadata) return "please resend the prompt";
    return metadata.total_supply !== undefined
      ? metadata.total_supply.toFixed(0)
      : "N/A";
  };
  const circulatingSupply = async (token) => {
    ...
    if (!metadata) return "please resend the prompt";
    return metadata.circulating_supply !== undefined
      ? metadata.circulating_supply.toFixed(0)
      : "N/A";
  };
  ...
  const maxSupply = async (token) => {
    ...
    if (!metadata) return "please resend the prompt";
    return metadata.max_supply !== undefined
      ? metadata.max_supply.toFixed(0)
      : "N/A";
  };
  ...
  const renderDisclaimerDialog = () => (
    ...
    >
      ...
      <DialogContent>
        ...
        >
          The information provided by this application is for informational
          purposes only and should not be considered as financial advice. Always
          conduct your own research and consult with a qualified financial
      ...
      {showAnnouncement && (
        <div style={styles.announcementBar}>
          <span style={styles.announcementText}>
            spatio AI can only answer questions related to your portfolio right
            now!
          </span>
          <CloseIcon
        ...
        >
          <div style={styles.walletAddress} onClick={handleWalletAddressClick}>
            {walletAddresses.length > 0
              ? `${walletAddresses[0].slice(0, 6)}...${walletAddresses[0].slice(
                  -4
                )}`
              : "Add Wallet"}
        ...
        <div style={styles.popup}>
          ...
          <div style={styles.walletAddressItem}>
            <input
              ...
              style={styles.popupInput}
            />
            <AddIcon onClick={handleAddWalletAddress} style={styles.addIcon} />
          </div>
          <button
